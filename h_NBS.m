...
%
% Last Modified by GUIDE v2.5 30-May-2010 16:53:20
%
...
for subj=1:length(NBS.GUI(1).subjects)
    ...
    if subj > length(NBS.CONFIG(subj))
        break;
    end
...
for i = SZ(2):SZ(2)
    ...
    for j = 1:SZ(3)
        ...
        for h = 1:size(RES,1)
            CMPARR(logical(CMPARR(:,1) == RES(h,1)),2) = str2double(cell2mat(ANSW(h,1)));
        end
...
str = get(handles.popupmenu1,'str');
val = get(handles.popupmenu1,'val');
[a b c] = fileparts(str{val});
disp(['User saved ', fullfile(pathname, filename)])
NBS.GUI.subjects = get(handles.popupmenuSearchResults,'str');
NBS.GUI.exams = get(handles.stimex,'str');
NBS.GUI.sequences = get(handles.showseq,'str');
NBS.GUI.textdata = evalin('base','textdata');
NBS.GUI.col2text = evalin('base',['NBS.GUI(' num2str(subj) ').col2text']);
NBS.GUI.col1text = evalin('base','col1text');
save(fullfile(pathname,filename),'NBS')
...
CRRGRP = get(handles.SBGRPS,'str');
TMPGRP = char(CRRGRP(get(handles.SBGRPS,'val'),1));


%Set selelected values as default answer for subsequent inputdlg
if any(strmatch('subgroups',CRRGRP))
    TMPGRPc = {[''];['']};
elseif ~isempty(strfind(TMPGRP,'OR'))
    TMPGRPc = {TMPGRP((strfind(TMPGRP,'AND:')+5) : (strfind(TMPGRP,'OR:')-4));...
        TMPGRP((strfind(TMPGRP,'OR:')+4) : length(TMPGRP))};
else
    TMPGRPc = {TMPGRP((strfind(TMPGRP,'AND:')+5) : length(TMPGRP)); ['']};
end

ADDGRP = inputdlg({char({'Add tags that define a subgroup:';' ';'AND: all criteria have to be found';...
    'OR: one of these criteria has to be found';' ';'AND:'}),'OR:'},'Subgroup definition',1,TMPGRPc);
if isempty(ADDGRP)
    return;
elseif ~isempty(cell2mat(ADDGRP(2,1)))
    ADDGRP = {['AND: ' char(ADDGRP{1,1}) ' - OR: ' char(ADDGRP{2,1})]};
elseif ~isempty(cell2mat(ADDGRP(1,1)))
    ADDGRP = {['AND: ' char(ADDGRP{1,1})]};
else
    ADDGRP = [];
end
        ...
        switch lower(char(NBS.ANALYSIS.MTRXhdr((get(handles.evaltype,'val') -1),1)))
            ...
            case 'paired - pulse'
                ...
                for z = 1:size(MTCHS,1)
                    ...
                    if ~isnan(TMP(1,1)) && ~isinf(TMP(1,1))
                        ...
                        disp(['Infinite or illegal result (NaN) for: ' char(CURRSUB(cell2mat(MTCHS(z,2)))) ...
                ...
                for z = 1:length(GRPS)
                    ...
                    ISI = unique(TMP(:,5));
                    for y = 1:length(ISI)
                        GRPMTRX(end+1,:) = [z ISI(y) mean(TMP(logical(TMP(:,5) == ISI(y)),4)) std(TMP(logical(TMP(:,5) == ISI(y)),4))];
                    end
                end
                    ...
                    for y = 1:length(ISI)
                        ...
                        for h = 1:length(GRPS)
                            if ~isempty(FITISI(logical(FITISI(:,1) == GRPS(h)), 4))
                                SUBMTRX(end+1,:) = [SUBIDX(z) h ISI(y) mean(FITISI(logical(FITISI(:,1) == GRPS(h)), 4))  std(FITISI(logical(FITISI(:,1) == GRPS(h)), 4))];
                            end
                        end
            ...
            case 'recruitment curve'
                ...
                for z = 1:size(MTCHS,1)
                    ...
                    if length(cell2mat(MTCHS(z,4))) == length(cell2mat(MTCHS(z,5)))
                        
                        TMP = cell2mat(MTCHS(z,[4 5]));
                        
                    elseif length(cell2mat(MTCHS(z,4))) > length(cell2mat(MTCHS(z,5)))
                        
                        TMP1 = cell2mat(MTCHS(z,[4]));
                        TMP2 = cell2mat(MTCHS(z,[5]));
                        
                        TMP = [TMP1(1:length(TMP2),:) TMP2(1:length(TMP2),:)];
                    ...
                    elseif length(cell2mat(MTCHS(z,4))) < length(cell2mat(MTCHS(z,5)))
                        ...
                        TMP = [TMP1(1:length(TMP1),:) TMP2(1:length(TMP1),:)];
                    ...
                    HLD = [];
                    for k = 1:length(INTS)
                        HLD(k,:) = [MEPmean(TMP(logical(TMP(:,8) == INTS(k)),CHN),1) INTS(k)];
                    end
                    ...
                    PLTMTRX((end+1):(end+size(HLD,1)),:) = [mtimes(ones(size(HLD,1),1),cell2mat(MTCHS(z,1:3))) HLD];
                    ...
                    for y = 1:length(INTS)
                        ...
                        for h = 1:length(GRPS)
                            if ~isempty(FITINTS(logical(FITINTS(:,1) == GRPS(h)), 4))
                                ...
                                SUBMTRX(end+1,:) = [SUBIDX(z) h INTS(y) MN MNSTD];
                            end
                        end
    ...
    case 'BOXPLOT - first level'
        ...
        switch lower(char(NBS.ANALYSIS.MTRXhdr((get(handles.evaltype,'val') -1),1)))
            ...
            case {'paired - pulse', 'threshold'}
                ...
                PLTMTRX = [];
                ANSW = questdlg('Use baseline or PP MEPs?','Choose...','baseline','PP','PP');
                for z = 1:size(MTCHS,1)
                    ...
                    %get baseline
                        TMP = TMP(logical(TMP(:,2) == 0),:);
                        
                    %in this case no mean is applied but all other changes
                    %like z-transform or outlier correction
                    TMP = MEPmean(TMP,1,'skip');
                    
                    for t = 1:length(TMP)
                        PLTMTRX(end+1,:) =  [cell2mat(MTCHS(z,1:3)) TMP(t,:)];
                    end
                end
                
                GRPMTRX = PLTMTRX(:,[1 5 4]);
                SUBMTRX = PLTMTRX(:,[2 1 5 4]);
                
                %GROUPSPEC MATRIX equals PLTMATRX
                
                %SUBJSPEC MATRIX equals PLTMATRX
                PLTMTRX = [];
                
                %% Do the same for the remainder data to calculate a ratio
                for z = 1:size(MTCHS,1)
                ...
                GRPMTRX_pp = PLTMTRX(:,[1 5 4]);
                SUBMTRX_pp = PLTMTRX(:,[2 1 5 4]);
            ...
            case 'recruitment curve'
                ...
                for z = 1:size(MTCHS,1)
                    ...
                    try
                        ...
                        TMP = [];
                        if length(cell2mat(MTCHS(z,4))) > length(cell2mat(MTCHS(z,5)))
                            TMP(:,[7 8]) = cell2mat(MTCHS(z,5));
                            PST = cell2mat(MTCHS(z,4));
                            TMP(:,1:6) = PST(1:size(TMP,1),:);
                        else
                            TMP(:,1:6) = cell2mat(MTCHS(z,4));
                            PST = cell2mat(MTCHS(z,5));
                            TMP(:,[7 8]) = PST(1:size(TMP,1),:);
                        end
                    end
                ...
                for z = 1:length(GRPS)
                    ...
                    for y = 1:length(INTS)
                        GRPMTRX(end+1,:) = [{GRPS(z)} {INTS(y)} {cell2mat(TMP(logical(cell2mat(TMP(:,4)) == INTS(y)),5))}];
                    end
                end
                    ...
                    for y = 1:length(ISI)
                        ...
                        for h = 1:length(GRPS)
                            if ~isempty(FITISI(logical(FITISI(:,1) == GRPS(h)), 4))
                                ...
                                SUBMTRX(end+1 : end + SZ,:) = [(ones(SZ,1) .* SUBIDX(z))...
                ...
                for i = 1:length(SUBIDX)
                    ...
                    for z = 1:size(GRPS,1)
                        ...
                        try
                            ...
                            TMP = [];
                            if length(cell2mat(HLD(logical(cell2mat(HLD(:,1)) == GRPS(z)),4))) > length(cell2mat(HLD(logical(cell2mat(HLD(:,1)) == GRPS(z)),5)))
                                TMP(:,[7 8]) = cell2mat(HLD(logical(cell2mat(HLD(:,1)) == GRPS(z)),5));
                                PST = cell2mat(HLD(logical(cell2mat(HLD(:,1)) == GRPS(z)),4));
                                TMP(:,1:6) = PST(1:size(TMP,1),:);
                            else
                                TMP(:,1:6) = cell2mat(HLD(logical(cell2mat(HLD(:,1)) == GRPS(z)),4));
                                PST = cell2mat(HLD(logical(cell2mat(HLD(:,1)) == GRPS(z)),5));
                                TMP(:,[7 8]) = PST(1:size(TMP,1),:);
                            end
                        end
...
switch char(CURRPLT(get(handles.PLTTYPE,'val'),1))
    case {'BAR','SCATTER'}
        ...
        if get(handles.PLTsbgrp,'val')
            ...
            for i = 1:length(GRPIDX)
                ...
                go = 0;                
                if any(strmatch('on',get(handles.orthogonalize,'checked'))) && go == 1
                    OFFST = -(min(CRRBARS(:,3)));
                else
                    OFFST = 0;
                end
                ...
                if max(abs(get(gca,'ylim'))) < 10
                    set(gca,'ylim',[(min(GRPMTRX(:,3) - GRPMTRX(:,4))-2) (max(GRPMTRX(:,3) + GRPMTRX(:,4)) + 2)]);
                else
                    set(gca,'ylim',[(min(GRPMTRX(:,3) - GRPMTRX(:,4))-10) (max(GRPMTRX(:,3) + GRPMTRX(:,4)) + 10)]);
                end
                
                %ADD USED VALUES TO LABLES
                XLBL = get(gca,'xticklabel');
                INTND = [];
                for u = 1:size(XLBL,1)
                    if any(strfind(XLBL(u,:),' '))
                        INTND(u) = strfind(XLBL(u,:),' ');
                    else
                        INTND(u) = size(XLBL,2) + 1;
                    end
                end
                
                for u = 1:size(XLBL,1)
                    ADDLBL = ['_(' num2str(roundn(CRRBARS(u,3),-2)) ')'];
                    XLBL(u,INTND(u) : INTND(u) + length(ADDLBL) - 1) = ADDLBL;
                end
                ...
                %linear fit checked?
                if get(handles.linfit,'val') == 1
                
                COFF = polyfit(CRRBARS(:,2),CRRBARS(:,3),1);
                R = corrcoef(CRRBARS(:,2),CRRBARS(:,3));
                R = R(1,2);
                PLTFIT(1,:) = COFF(1,1).*(CRRBARS(1,2) : (CRRBARS(end,2)-CRRBARS(1,2))/10 : CRRBARS(end,2)) + COFF(1,2);
                PLTFIT(2,:) = (CRRBARS(1,2) : (CRRBARS(end,2)-CRRBARS(1,2))/10 : CRRBARS(end,2));
                plot(PLTFIT(2,:),PLTFIT(1,:)+OFFST,'r.');
                title(char([SGRP(i); {['s: ' num2str(COFF(1,1)) ' i: ' num2str(COFF(1,2)) '; R: ' num2str(R)]}]));
                PSH = get(gca,'Position');
                set(gca,'Position',PSH + [0 -0.05 0 0]);
            ...
            for i = 1:length(SUBIDX)
                ...
                for k = 1:length(GRPIDX)
                    ...
                    if get(handles.linfit,'val') == 1
                        ...
                        PLTFIT(2,:) = (CRRBARS(1,2) : (CRRBARS(end,2)-CRRBARS(1,2))/10 : CRRBARS(end,2));
                        plot(PLTFIT(2,:),PLTFIT(1,:)+OFFST,'r.');
                        title([{[char(CURRSUB(SUBIDX(i),1)) ' - GRP ' num2str(k)]} ; {['s: ' num2str(COFF(1,1)) ' i: ' num2str(COFF(1,2)) '; R: ' num2str(R)]}]);
                        PSH = get(gca,'Position');
                        set(gca,'Position',PSH + [0 -0.05 0 0]);
    ...
    case {'BOXPLOT - first level','BOXPLOT - second level'}
        ...
        for i = 1:length(GRPIDX)
            ...
            hold on
            %%% get means
            TMPgrp_un = unique(cell2mat(TMPgrp));
            try
            TMPdat = cell2mat(TMP);
            catch
                TMPdat = [];
                for t = 1:length(TMPgrp_un)
                    TMPdat(end+1,:) = [cell2mat(TMP(t,1:2)) mean(cell2mat(TMP(t,3)))];
                end
            end
            MNS = 'mean: ';
            for t = 1:length(TMPgrp_un)
                MNS(end+1 : end + 2 + length(num2str(roundn(mean(TMPdat(TMPdat(:,2) == TMPgrp_un(t),3)),-2)))) = [num2str(roundn(mean(TMPdat(TMPdat(:,2) == TMPgrp_un(t),3)),-2)) ', '];
            end
            
            STDS = 'std: ';
            for t = 1:length(TMPgrp_un)
               STDS(end+1 : end + 2 + length(num2str(roundn(std(TMPdat(TMPdat(:,2) == TMPgrp_un(t),3))),-2))) = [num2str(roundn(std(TMPdat(TMPdat(:,2) == TMPgrp_un(t),3)),-2)) ', '];
            end
            
            if exist('GRPMTRX_pp','var')
                TMPpp = GRPMTRX_pp;
                GRPMTRX_pp = GRPMTRX_pp(GRPMTRX_pp(:,1) == GRPIDX(i),:);
                TMPhld = GRPHLD;
                GRPHLD = GRPHLD(GRPHLD(:,1) == GRPIDX(i),:);
                for t = 1:length(TMPgrp_un)
                    RTmn(t) = mean(GRPMTRX_pp(GRPMTRX_pp(:,2) == TMPgrp_un(t),3)) / mean(GRPHLD(GRPHLD(:,2) == 0,3));
                end
                GRPMTRX_pp = TMPpp;
                GRPHLD = TMPhld;
            else
            ...
            title([char(SGRP(i)) ' ' MNS ' '  STDS ' ' num2str(RTmn)]);
            
            if get(handles.evaltype,'val') == (strmatch('threshold',NBS.ANALYSIS.MTRXhdr(:,1)) + 1)
                set(gca,'ylim',[(min(cell2mat(GRPMTRX(:,3))) -20) (max(cell2mat(GRPMTRX(:,3))) + 20)]);
            elseif any(strmatch('on',get(handles.orthogonalize,'checked')))
                set(gca,'ylim',[(min(cell2mat(GRPMTRX(:,3))) -5) (max(cell2mat(GRPMTRX(:,3))) + 5)]);
            else
        ...
        if get(handles.PLTsingle,'val') == 1
            ...
            [s,v] = listdlg('PromptString','Select a file:',...
                'SelectionMode','multple',...
                'ListString',CURRSUB(SUBIDX),'initialvalue',get(handles.SBJTS,'val'));
            SUBIDX = SUBIDX(s);
            [s,v] = listdlg('PromptString','Select a file:',...
                'SelectionMode','multple',...
                'ListString',get(handles.SBGRPS,'str'),'initialvalue',get(handles.SBGRPS,'val'));
            GRPIDX = GRPIDX(s);
            ...
            for i = 1:length(SUBIDX)
                ...
                for k = 1:length(GRPIDX)
                    ...
                    boxplot(cell2mat(TMP(:,3)),TMPgrp);
                    hold on
                    title([char(CURRSUB(SUBIDX(i),1)) ' - GRP ' num2str(k)]);
                    if get(handles.evaltype,'val') == (strmatch('threshold',NBS.ANALYSIS.MTRXhdr(:,1)) + 1)
                        set(gca,'ylim',[(min(cell2mat(GRPMTRX(:,3))) -20) (max(cell2mat(GRPMTRX(:,3))) + 20)]);
                    else
                        set(gca,'ylim',[(min(cell2mat(GRPMTRX(:,3))) -200) (max(cell2mat(GRPMTRX(:,3))) + 200)]);
    ...
    case 'ANOVA - first level'
        ...
        switch lower(char(NBS.ANALYSIS.MTRXhdr((get(handles.evaltype,'val') -1),1)))
            ...
            case 'recruitment curve'
                ...
                for m = 1:size(GRPMTRX,1)
                    TMPAMPS = cell2mat(GRPMTRX(m,3));
                    for n = 1:length(TMPAMPS)
                        PLTTMP(end+1,1:3) = [{cell2mat(GRPMTRX(m,1))} {cell2mat(GRPMTRX(m,2))} {TMPAMPS(n)}];
                    end
                end
                ...
                %%TMP by rob
                SUBMTRX = SUBMTRX(cell2mat(SUBMTRX(:,3)) ~= 150,:);
            ...
            case 'recruitment curve'
                ...
                for i = 1:length(SUBIDX)
                    ...
                    for z = 1:size(GRPS,1)
                        ...
                        elseif length(cell2mat(HLD(logical(cell2mat(HLD(:,1)) == GRPS(z)),[4]))) > length(cell2mat(HLD(logical(cell2mat(HLD(:,1)) == GRPS(z)),[5])))
                            
                            TMP1 = cell2mat(HLD(logical(cell2mat(HLD(:,1)) == GRPS(z)),[4]));
                            TMP2 = cell2mat(HLD(logical(cell2mat(HLD(:,1)) == GRPS(z)),[5]));
                            TMP = [TMP1(1:length(TMP2),:) TMP2(1:length(TMP2),:)];
                        ...
                        if ~isempty(TMP)
                            ...
                            for k = 1:length(INTS)
                                ...
                                PLTMTRX(end+1,:) = [{SUBIDX(i)} {GRPS(z)} {INTS(k)} {MEPmean(cell2mat({TMP(logical(TMP(:,8) == INTS(k)),CHN)}))}];
                            end
            ...
            case 'thresholds'
                ...
                for t = 1:length(SUBIDX)
                    ...
                    for z = 1:length(GRPS)
                        ...
                        SUBMTRX(end+1 : end + SZ ,:) = [(ones(SZ,1) .* SUBIDX(t)) (ones(SZ,1) .* z) ...
                            ones(SZ,1) TMP(TMP(:,1) == z,4)];
                    end
...
switch char(CURRPLT(get(handles.STATchs,'val'),1))
    ...
    case {'ttest - paired - two-tailed', 'ttest - unpaired - two-tailed',...
        ...
        for i = 1:length(TMSPRM)
            ...
            for j = 1:length(GRPS)
                ...
                for k = 1:length(SUBIDX)
                    ...
                    if max(GT) == 3  %i.e. all conditions exist for this subject
                        ...
                        try
                        disp(['Skipping following condition: SUB: ' char(CURRSUB(SUBIDX(k))) ', GROUP:' ...
        ...
        RES(end+1,1) = {' '};
        RES(end+1,1) = {char(CURRPLT(get(handles.STATchs,'val'),1))};
        RES(end+1,1) = {' '};
        ...
        %% SLOPES/INTCPT ARE TESTED BELOW
        %%%%%%%
        ...
        for i = 1:length(TMSPRM)
            ...
            for j = 1:length(SUBIDX)
                ...
                for k = 1 : length(GRPS)
                    ...
                    %see if all GRPS exist for this subject, if not,
                    %remove!
                    
                    if sum(GRPS(k) == TMP(TMP(:,2) == SUBIDX(j),3)) == 0 && ...
                            (any(strmatch('ttest - paired - two-tailed', char(CURRPLT(get(handles.STATchs,'val'),1)))) || ...
                            any(strmatch('Wilcoxon signed rank test', char(CURRPLT(get(handles.STATchs,'val'),1)))))
                        
                        SUBS = SUBS(SUBS ~= SUBIDX(j),:);
                        TMP = TMP(TMP(:,2) ~= SUBIDX(j),:);
                        
                        %% falls parameter in zwei konditionen
                        %% unterschiedlich häufig gemessen wurden und
                        %% trotzdem in einem paired ttest verglichen werden
                        %% sollen
                        
                    elseif sum(GRPS(k) == TMP(TMP(:,2) == SUBIDX(j),3)) > 1 && ...
                            (any(strmatch('ttest - paired - two-tailed', char(CURRPLT(get(handles.STATchs,'val'),1)))) || ...
                            any(strmatch('Wilcoxon signed rank test', char(CURRPLT(get(handles.STATchs,'val'),1)))))
                        
                        if k == 1 && sum(GRPS(k+1) == TMP(TMP(:,2) == SUBIDX(j),3)) > 1
                            errordlg({['Subject ' char(CURRSUB(SUBIDX(j))) ' has double entries for'...
                                ' both entered conditions and data can therefore not be analysed!'];...
                                'Consider adding the subject a second time and changing tags to clarify belonging sequences'});
                            uiwait;
                            return;
                            
                        else
                            CURRMTCH = find(TMP(:,2) == SUBIDX(j));
                            CURRMTCH = CURRMTCH(GRPS(k) == TMP(TMP(:,2) == SUBIDX(j),3));
                            
                            for d = 2:length(CURRMTCH) %-1 because first one can remain
                                NWIDX = max(SUBS) + 1;
                                CURRSUB(end+1,1) = CURRSUB(SUBIDX(j));
                                SMPLMTCH = find(TMP(:,2) == SUBIDX(j));
                                SMPLMTCH = SMPLMTCH(GRPS(find([1 2] ~= k)) == TMP(TMP(:,2) == SUBIDX(j),3));
                                TMP(end+1,:) = [TMP(SMPLMTCH,1) NWIDX TMP(SMPLMTCH,[3 4])];
                                TMP(CURRMTCH(d),2) = NWIDX;
                                SUBS(end+1) = NWIDX;
                            end
                        end
            ...
            if length(SUBS) < 2
                ...
                disp(['Cannot compare PARAM: ' num2str(TMSPRM(i)) ' for GROUP: ' char(SGRP(k)) ...
                ...
                switch char(CURRPLT(get(handles.STATchs,'val'),1))
                    ...
                    case 'ttest - paired - two-tailed'
                        ...
                        if length(GRPS) < 2
                            disp('Cannot compare as just one group given...');
                            return;
                        end
                    ...
                    case 'Wilcoxon signed rank test'
                        ...
                        [p h] = signrank(TMP(TMP(:,3) == GRPS(1),4),TMP(TMP(:,3) == GRPS(2),4),'alpha',0.05,'method','exact');
                    ...
                    case 'Mann-Whitney U-Test'
                        ...
                        [p h] = ranksum(TMP(TMP(:,3) == GRPS(1),4),TMP(TMP(:,3) == GRPS(2),4));
                end
        ...
        if get(handles.linfit,'val') == 1
            ...
            for j = 1:length(SUBIDX)
                ...
                TMP = TTMTRX(TTMTRX(:,2) == SUBIDX(j),:);

                for k = 1:length(GRPS)
                    TMP1 = TMP(TMP(:,3) == GRPS(k),:);
                    
                    if size(TMP1,1) < length(TMSPRM)
                        disp(['Subj: ' char(CURRSUB(SUBIDX(j))) ' is lacking some TMSPRM for fit of COND: ' num2str(k) '...']);
                    end
                    
                    COFF = polyfit(TMP1(:,1),TMP1(:,4),1);
                    %R = corrcoef(TMP1(:,1),TMP1(:,4));
                    %R = R(1,2);
                    
                    SLP(j,k) = COFF(1,1);
                    INT(j,k) = COFF(1,2);
                    TMPIDX = abs(sum(TMP1(logical((TMP1(:,1) == 110) + (TMP1(:,1) == 120)),4)))/abs(sum(TMP1(logical((TMP1(:,1) == 140) + (TMP1(:,1) == 130)),4)));
                    
                    if ~(isnan(TMPIDX)) && ~(isinf(TMPIDX))
                    SLPIDX(j,k) = TMPIDX;
                    end
            ...
            for j = 1:3
                ...
                %put SLP as INT to make evaluation easier and reduce
                %programing
                if j == 2
                    SLP = INT;
                ...
                switch char(CURRPLT(get(handles.STATchs,'val'),1))
                    ...
                    case 'ttest - paired - two-tailed'
                        ...
                        [h p] = ttest(SLP(:,1),SLP(:,2),0.05,'both');
                        
                    case 'Wilcoxon signed rank test'
                        
                        [p h] = signrank(SLP(:,1),SLP(:,2),'alpha',0.05,'method','exact');
                        
                    case 'ttest - unpaired - two-tailed'
                        
                        [h p] = ttest2(SLP(:,1),SLP(:,2),0.05,'both');
                        
                    case 'Mann-Whitney U-Test'
                        
                        [p h] = ranksum(SLP(:,1),SLP(:,2));
                end
                
                if exist('p','var') == 1
                    if j == 1
                        RES(end+1,1) = {['PARAM: SLOPE | SUBJ: ' DSPSUB ,' | p = ' num2str(p)]};
                    elseif j == 2
                        RES(end+1,1) = {['PARAM: INTERCEPT | SUBJ: ' DSPSUB ,' | p = ' num2str(p)]};
                    else
                        RES(end+1,1) = {['PARAM: SLPIDX ' num2str(mean(SLPIDX(:,1))) ' vs. ' num2str(mean(SLPIDX(:,2))) ' | SUBJ: ' DSPSUB ,' | p = ' num2str(p)]};
                    end
                else
                    RES(end+1,1) = {['PARAM: ' num2str(TMSPRM(i)) ' | SUBJ: ' DSPSUB ,' | ERROR!']};
                end
...
for k = 1:size(SBGRPs,1)
    ...
    CURRGRP_AND = regexp(TMPGRPc{1,1},' ','split');
    CURRGRP_OR = regexp(TMPGRPc{2,1},' ','split');
    for l = 1:length(CURRGRP_AND)
        if ischar(cell2mat(CURRGRP_AND(1,l)))
            CURRGRP_AND(1,l) = lower(CURRGRP_AND(1,l));
        end
    end
    for l = 1:length(CURRGRP_OR)
        if ischar(cell2mat(CURRGRP_OR(1,l)))
            CURRGRP_OR(1,l) = lower(CURRGRP_OR(1,l));
        end
    end
    ...
    for i = 1:size(ARR,2)
        ...
        for j = 1:size(ARR,3)
            ...
            %check for matches with SBGRP
            cnt = 0;
            for h = 1:length(CURRGRP_AND)
                
                
                if get(handles.MTCHEX,'val') == 0 %default
                    MTCHRES = strmatch(CURRGRP_AND(1,h),CURRdesc);
                else
                    MTCHRES = strmatch(CURRGRP_AND(1,h),CURRdesc,'exact');
                end
            ...
            if cnt == length(CURRGRP_AND)
                
                if isempty(TMPGRPc{2,1})
                MTCHS(end + 1,:) = {k i j};
                else
                    cnt = 0;
                    for h = 1:length(CURRGRP_OR)
                        
                        
                        if get(handles.MTCHEX,'val') == 0 %default
                            MTCHRES = strmatch(CURRGRP_OR(1,h),CURRdesc);
                        else
                            MTCHRES = strmatch(CURRGRP_OR(1,h),CURRdesc,'exact');
                        end
...
MTCHSL = size(MTCHS,2);
if any(strmatch('off',get(handles.orthogonalize,'checked'))) || ~any(strmatch('RAWAMP',PRM));
    ...
    SUBS = unique(cell2mat(MTCHS(:,2)));
    ZTRDATA = {};
    
    for i = 1:length(SUBS)
        CNMTRX = {};
        SEQ = cell2mat(MTCHS(cell2mat(MTCHS(:,2)) == SUBS(i),3));
        IDX = [];
        
        for j = 1:sum(cell2mat(MTCHS(:,2)) == SUBS(i))
            CNMTRX(j,1) = ARR(1,SUBS(i),SEQ(j),strmatch('RAWAMP',NBS.ANALYSIS.MTRXhdr(:,4)));
            IDX(j) = length(cell2mat(ARR(1,SUBS(i),SEQ(j),strmatch('RAWAMP',NBS.ANALYSIS.MTRXhdr(:,4)))));
        end
        
        CNMTRX = zscore(cell2mat(CNMTRX));
        
        for j = 1:length(IDX)
            if j == 1
                ZTRDATA(i,j) = {CNMTRX(1:IDX(j),:)};
            else
                ZTRDATA(i,j) = {CNMTRX((sum(IDX(1:(j-1))) + 1) : (sum(IDX(1:(j-1))) + IDX(j)),:)};
            end
        end
    end
    ...
    for i = 1:length(PRM)
        ...
        for j = 1:size(MTCHS,1)
            ...
            if ~any(strmatch('RAWAMP',PRM(i)))
                
                MTCHS(j,(MTCHSL + i)) = ARR(1,cell2mat(MTCHS(j,2)),cell2mat(MTCHS(j,3)),strmatch(PRM(i),NBS.ANALYSIS.MTRXhdr(:,4)));
                
            else
                
                SIDX = find(j == find(cell2mat(MTCHS(j,2)) == cell2mat(MTCHS(:,2))));
                MTCHS(j,(MTCHSL + i)) = ZTRDATA(cell2mat(MTCHS(j,2)) == SUBS,SIDX);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [MN MNSTD] = MEPmean(MTX,VL,SKP)

md = 0; %use median instead of mean
...
for i = 1:size(MTX,2)
    ...
    switch get(handles.MEPoutliers,'Checked')
        ...
        case 'on'
            ...
            if nargin <= 2
                ...
                MNtmp = MTX(TMP >= (QUANTILE(TMP,0.25) - LMT*FKT),i);
                MNtmp = MNtmp(MNtmp <= (QUANTILE(TMP,0.75) + LMT*FKT),1);
            ...
            elseif nargin > 2
                LMT = QUANTILE(TMP(:,1),0.75) - QUANTILE(TMP(:,1),0.25); %i.e. cut off to be used
                FKT = 0;
                
                MNtmp = MTX(TMP(:,1) >= (QUANTILE(TMP(:,1),0.25) - LMT*FKT),:);
                MNtmp = MNtmp(MNtmp(:,1) <= (QUANTILE(TMP(:,1),0.75) + LMT*FKT),:);
    ...
    if nargin > 2
        MN = MNtmp;
        %NOTE: if an error occurs using MEPmean function bear in mind that if nargin > 2 only the first column is considered');
        return;
    end
    
    if md == 1 %use median or mean? 1 == median, 0 == mean
...
function CHN = CHNID(TMP)

%%for now, ignore channels 1,3 and 4
%TMP has to contain the six EMG channles; gives the strongest channel
handles = evalin('base','handles');
...
DEFCHN = PRMS{1,1};
DEFCHN = DEFCHN(strfind(DEFCHN,'[')+1 : strfind(DEFCHN,']')-1);
DEFCHN = str2num(DEFCHN);

if any(strmatch(get(handles.orthogonalize,'Checked'),'on'))
    cnt = 0;
    for i = 1:length(DEFCHN)
        cnt(i) = numel(unique(TMP(:,DEFCHN(i))));
    end
    CHN = DEFCHN(cnt == max(cnt));
    if numel(CHN) > 1
        disp('multiple channels have been identified as highest, choosing first by default..');
        CHN = CHN(1,1);
    end
    return;
end

%% search for consecutive 0's in defined channels;
%% reference if more than 20% of max size are
%% consecutive 0's
cnt = 0;
for n = 1:length(DEFCHN)
    cnt_z = 0;
    for m = 1:size(TMP,1)
        
        cnt_z = cnt_z + 1;
        if TMP(m,DEFCHN(n)) > 0 || m == size(TMP,1)
            cnt(n,m) = cnt_z;
            cnt_z = 0;
        end
    end
...
OFFS = 0.20; % how many consecutive 0 results make a channel to be ignored (i.e. 0.2 equals 20% of array size)

if (sum(max(cnt,[],2) > (size(TMP,1) * OFFS)) < length(DEFCHN)) && sum(max(cnt,[],2) > (size(TMP,1) * OFFS)) > 0
    DEFCHN = DEFCHN(~(max(cnt,[],2) > (size(TMP,1) * OFFS)));
else
    DEFCHN = DEFCHN(~(max(cnt,[],2) > min(max(cnt,[],2))*2));
end

if max(max(TMP(:,DEFCHN))) == 1000
    
    %avoid having artificially created a channel maximum of 1000 by
    %introduction of 1000µV reference stimuli in paired-pulse stimuli
    %lacking reference values
    TMP = TMP(TMP(:,DEFCHN(1,1)) ~= 1000,:);
end

MNS = [];
for i = 1:length(DEFCHN)
    MNS(i) = mean(TMP(:,DEFCHN(i)));
end

CHN = DEFCHN(find(MNS == max(MNS)));

if length(CHN) > 1
    %%try again considering all channels
    DEFCHN = [1 2 3 4 5 6];
    if any(strmatch(get(handles.orthogonalize,'Checked'),'on'))
        cnt = 0;
        for i = 1:length(DEFCHN)
    ...
    CHN = DEFCHN(find(MNS == max(MNS)));
    disp('New search among all channels started as no channel among given ones identified..');
end

if length(CHN) > 1
    CHN = CHN(1,1);
    disp('A random channel had to be selected..');
end
