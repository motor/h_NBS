...
% Last Modified by GUIDE v2.5 04-Dec-2009 19:10:46
...
if isempty(str)
    ...
    set(handles.seqinf,'string',{seqstr{:},[GUI(1,val).sequencesindices{subj} ' (' nrev ')']});
...
for i = SZ(2):SZ(2) %go through last subject
    ...
    %%Correct SZ(3) as it might be false high cause of other subjects
    %%with more sequences
    SZ(3) = length(CURRSEQ);
        ...
        for j = 1:SZ(3) %go through sequences
            ...
            if sum(MSO(:,1) == 0) ~= size(MSO,1) && sum(MSO(:,2) == 0) ~= size(MSO,1)
                ...
                elseif refcnt == 0
                    ...
                    helpdlg(['Reference NOT found for ' char(TMP(1,i,j,1)) ' , please choose a reference from following sequences...']);
                    uiwait;
                    REFSEQ = listdlg('PromptString','Choose a reference sequence',...
                        'ListSize',[300 300],'SelectionMode','single',...
                        'ListString',CURRSEQ);
                    
                    if isempty(REFSEQ)
                        errordlg('No valid reference selected, please enter a reference to be used');
                        uiwait;
                        %%% Manuelle Eingabe fuer eine Referenz
                        try
                            ANSWER = str2num(cell2mat(INPUTDLG('Please enter the desired reference amplitude (µV):','PP Reference',1,{'1000'})));
                        catch
                    ...
                    if refcnt == 0
                        errordlg('No non-zero stimintensity. please enter a reference to be used');
                        uiwait;
                        ...
                        if abs(stimint) > 100 %wenn unterschied zwischen stimintensities > 2 ist ref ungeeignet
                            errordlg('Max MSO differs more than 2 units! Not a suitable reference, action cancelled...');
                            uiwait;
                            return;
                        else
                            addMSO = cell2mat(TMP(1,i,REFSEQ,5));
    ...
    for sess = sessind;
        ...
        for cnd = condind;
            ...
            if anlyss.scatter == 1 && NBS.defaults.ctrl% scatter plots of raw data
                ...
                subplot(2,2,1),
                scatter([LOC(1,:)],[LOC(3,:)],200,AMPS/1000,'filled');hold on
...
for sess = 1:size(A1,1);
    ...
    A_thresh(A_thresh<50) = 0;
    A_pastespecial{sess}(:,conds) = A_tresh;
...
switch str{val}
    ...
    case '- MappingResults'
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        warning off MATLAB:xlswrite:AddSheet
        xlswrite(filename,E,sheetname,'A32')
...
NBS = evalin('base','NBS');
subj = get(handles.popupmenuSearchResults,'val');
ButtonName = questdlg('What to do?', ...
    'Sequence Question', ...
    'reorganize', 'delete', 'reorganize');
...
for newsubj = 1:size(COMPARE.path_file,1)
    ...
    if isempty( NBS.GUI(1,newsubj).sequences);
        ...
        for sess = 1:size(ind_get,1);
            ...
            seq_preview = get(handles.seq_preview,'val');
...
for i = 1:1:size(content,1)
    cont(i,1) = {getfield(content,{i,1},'name')};
end
for i = 1:1:size(cont,1)
    found = strfind(char(cont(i,:)),ext);
    if isempty(found) ~= 1
        try
            match(end+1,1) = cont(i,1);
        catch
            match(1,1) = cont(i,1);
        end
    end
...
try
    for i = 1:1:size(match,1)
        COMPARE.path_file((end+1),1:2) = {dirname, char(match(i,1))};
    end
catch
    if exist('match') == 0
        errordlg('No excel files found...');
        return;
    else
        for i = 1:1:size(match,1)
            if i == 1
...
for i = 1:1:size(INDX,1)
    try strmatch('Patient Name:',char(INDX(i,1).rawmatrix(9,1)))
        h_hld = char(INDX(i,1).rawmatrix(9,1));
    catch
        disp(lasterr)
        uiwait(msgbox(['something is wrong with subject: ' cell2mat(temp(i,2)),...
...
set(handles.popupmenuSearchResults,'val',length(SEQ));
assignin('base','NBS',NBS)
assignin('base','COMPARE',COMPARE)
...
switch get(handles.evaltype,'val')
        case 2
            NBS.ANALYSIS.MTRX(strmatch('thresholds',NBS.ANALYSIS.MTRXhdr),:,:,:) = ARR;
            
        case 3
            NBS.ANALYSIS.MTRX(strmatch('mean',NBS.ANALYSIS.MTRXhdr),:,:,:) = ARR;
            
        case 4
            NBS.ANALYSIS.MTRX(strmatch('paired pulse',NBS.ANALYSIS.MTRXhdr),:,:,:) = ARR;
            
        case 5
            NBS.ANALYSIS.MTRX(strmatch('recruitment curves',NBS.ANALYSIS.MTRXhdr),:,:,:) = ARR;
end
...
for i = 1:length(SBGRPs)
    TMPMEAN = {};
    TMPMSO = {};
    TMPISI = {};
    TMPAMPS = {};
    
    GRPMTCH = MTCHS(MTCHS(:,1) == i,:);
    
    for j = 1:size(GRPMTCH,1)
        TMPMEAN(end + 1,:) = ARR(1,GRPMTCH(j,2),GRPMTCH(j,3),2);
        TMPMSO(end + 1,:) = ARR(1,GRPMTCH(j,2),GRPMTCH(j,3),5);
        TMPISI(end + 1,:) = ARR(1,GRPMTCH(j,2),GRPMTCH(j,3),6);
        TMPAMPS(end + 1,:) = ARR(1,GRPMTCH(j,2),GRPMTCH(j,3),7);
    end
    
    ALLMEAN(end + 1,1) = {TMPMEAN};
    ALLMSO(end + 1,1) = {TMPMSO};
    ALLISI(end + 1,1) = {TMPISI};
    ALLAMPS(end + 1,1) = {TMPAMPS};
end
...
switch get(handles.evaltype,'val')
    ...
    case 4
        ...
        PLTmtrx = []; %[GROUP relativeMEAN ISI]
        for i = 1:length(SBGRPs)
            ...
            if isempty(AMPS)
                disp(['No results for subgroup ' char(SBGRPs(i,1)) '...']);
            end
            
            
            for k = 1:length(AMPS)
                
                TMPisi = cell2mat(ISI(k,1));
                TMPmso = cell2mat(MSO(k,1));
                AMPSpp = cell2mat(AMPS(k,1));
                
                if ~isempty(AMPSpp) %falls arrays unterschiedlich groß sind
                    
                    %get highest response (not in future, find another
                    %solution, e.g. by edf files)
                    if min(find(TMPmso == 0)) < size(TMPmso,1)
                        REFCOL = 1;
                    else
                        REFCOL = 2;
                    end
                    
                    AMPSreg = AMPSpp(TMPmso(:,REFCOL) ~= 0,:);
                    FNG = max(mean(AMPSreg,1)) == mean(AMPSreg,1); % channel to be used
                    AMPSreg = AMPSreg(:,FNG);
                    
                    AMPSref = AMPSpp(TMPmso(:,REFCOL) == 0,:);
                    AMPSref = AMPSref(:,FNG);
                    
                    PLTmtrx(end + 1,:) = [i mean(AMPSreg)/mean(AMPSref) max(TMPisi)];
...
switch get(handles.PLTTYPE,'val');
    ...
    case {1, 2}
        ...
        %------------
        % paired-pulse
        if any(exist('PLTmtrx','var')) % i.e. paired-pulse
            ...
            if get(handles.PLTsbgrp,'val')
                ...
                SUBGRP = unique(PLTmtrx(:,1));
                
                for i = 1:length(SUBGRP)
                    
                    CURRGRP = PLTmtrx(PLTmtrx(:,1) == SUBGRP(i),:);
                    ISIav = unique(CURRGRP(:,3));
                    
                    PLT = [];
                    for j = 1:length(ISIav)
                        
                        PLT(end + 1,:) = mean(CURRGRP(CURRGRP(:,3) == ISIav(j),:),1);
                    end
                    
                    if get(handles.PLTTYPE,'val') == 1
                        subplot(1,length(SUBGRP),i),  bar(PLT(:,3),PLT(:, 2));
                    else
                        subplot(1,length(SUBGRP),i),  scatter(PLT(:,3),PLT(:, 2));
                    end
                    title(gca,char(SBGRPs(i,1)));
            ...
            %Plot subjects
            if get(handles.PLTsingle,'val')
                SBLIST = get(handles.SBJTS,'str');
                ...
                SUBJTS = unique(MTCHS(:,2));
                
                for i = 1:length(SUBGRP)
                    
                    for k = 1:length(SUBJTS)
                        
                        CURRSUB = PLTmtrx(PLTmtrx(:,1) == SUBGRP(i),:);
                        MTCHStmp = MTCHS(PLTmtrx(:,1) == SUBGRP(i),:);
                        CURRSUB = CURRSUB(MTCHStmp(:,2) == SUBJTS(k),:);
                        ISIav = unique(CURRSUB(:,3));
                        
                        PLT = [];
                        for j = 1:length(ISIav)
                            
                            PLT(end + 1,:) = mean(CURRSUB(CURRSUB(:,3) == ISIav(j),:),1);
                        end
                        
                        if get(handles.PLTTYPE,'val') == 1
                            subplot(length(SUBGRP),length(SUBJTS),(k + (i-1)*length(SUBJTS))),  bar(PLT(:,3),PLT(:, 2));
                        else
                            subplot(length(SUBGRP),length(SUBJTS),(k + (i-1)*length(SUBJTS))),  scatter(PLT(:,3),PLT(:, 2));
                        end
                        title(gca,[char(SBLIST(k,1)) ' - ' char(SBGRPs(i,1))]);
...
try % there might be no data for the respective evaluation --> skip
    PRESZ = size(NBS.ANALYSIS.subjects,1);
    IDX = ones(1,PRESZ);
    ...
    switch get(handles.evaltype,'val')
        ...
        case 3
            EXAMTAG = 'mean';
        case 4
            EXAMTAG = 'paired pulse';
        case 5
            EXAMTAG = 'recruitment';
    end
    
    LAYER = strmatch(EXAMTAG,NBS.ANALYSIS.MTRXhdr);
    
    %Entfernen aller subjects dieser Untersuchung, werden unten wieder
    %angefuegt
    SBJCT = NBS.ANALYSIS.subjects(strmatch(EXAMTAG,NBS.ANALYSIS.subjects(:,2)),:);
    
    IDX(strmatch(EXAMTAG,NBS.ANALYSIS.subjects(:,2))) = 0;
    NBS.ANALYSIS.subjects = NBS.ANALYSIS.subjects(logical(IDX),:);
...
SBJCT = SBJCT(logical(~IDX),:);

NBS.ANALYSIS.subjects = [NBS.ANALYSIS.subjects; SBJCT];

% mit leeren cells ueberschreiben
...
switch char(asysindx(chkindx,1))
    ...
    case 'mean' %MEAN
        ...
        for i = 1:length(TGS)
            ...
            NBS.ANALYSIS.MTRX(chkindx, SUBSOFAR, i,1) = TGS(i,1);
            NBS.ANALYSIS.MTRX(chkindx, SUBSOFAR, i, 2) = {mean(AMPS(:,[1 3 5 7 9 11]))}; % MEAN
...
for i = 1:SZ(2)
    ...
    elseif  isempty(TMPidx) && i == SZ(2)% subject does not have any valid loaded sequence
        ...
        SZ = size(ARR);
        stpbck = 1; %jump a step back as successive subject move in the position of the removed subject
    end
end


%%% SUBGROUP definition
function [MTCHS SBGRPs] = SBGRP(ARR)
...
%check if empty
if any(strmatch('subgroups',SBGRPs))
